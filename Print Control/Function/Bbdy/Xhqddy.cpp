		...
		while (xml.FindElem("NewPage"))
		{
			...
			if (xml.FindElem("PageData"))
			{
				...
				while (xml.FindElem("Item"))
				{
					...
					PaintTile(nFontSize, strFontName, itemRect, strText, z, fc, 4, 2, 2);				
				}
				xml.OutOfElem();
			}
...
CString CXhqddy::GenerateItemMXXml(XHQD_BBXX bbxx)
{
	...
	for (pos = bbxx.st_lXhqd_qdxx.begin(); pos != bbxx.st_lXhqd_qdxx.end(); pos++)
	{
		...
		xywhsf(pos->xmXh, x0, _y, XU_W, nLY, LS_9, FS, AM_ZC_CHEKC | LINE_STATE_L);
		xywhsf(pos->xmSpmc, x1, _y, MC_W, nLY, LS_9, FS, AM_ZC_CHEKC | LINE_STATE_L);
		xywhsf(pos->xmGgxh, x2, _y, XH_W, nLY, LS_9, FS, AM_ZC_CHEKC | LINE_STATE_L);
