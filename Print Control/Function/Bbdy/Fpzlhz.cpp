...
LONG CFpzlhz::PrintQD(LPCSTR billxml, CString bblx)
{
...
FPZLHZ_BBXX CFpzlhz::ParseFpmxFromXML(LPCTSTR inXml, BBDY bbdy)
{
	...
	{
		bbxx.st_nTjxxCount = atoi(xml.GetAttrib("count"));
		xml.IntoElem();
		for (int i = 0; i < bbxx.st_nTjxxCount; i++)
		{
			xml.FindElem("group");
...
CString CFpzlhz::GenerateItemMXXml(FPZLHZ_BBXX bbxx)
{
	...
	for (pos = bbxx.st_lFpzlhzTjxx.begin(); pos != bbxx.st_lFpzlhzTjxx.end(); pos++)
	{
		...
		xywhsf(pos->xmHj, nShiting, i * nLY + ny, XM_1, nLY, LS_9, FS, AM_ZC);
		nShiting += XM_1;

		xywhsf(pos->xmS1, nShiting, i * nLY + ny, XM_2, nLY, LS_9, FS, AM_ZC);
		nShiting += XM_2;

		xywhsf(pos->xmS2, nShiting, i * nLY + ny, XM_3, nLY, LS_9, FS, AM_ZC);
		nShiting += XM_3;

		xywhsf(pos->xmS3, nShiting, i * nLY + ny, XM_4, nLY, LS_9, FS, AM_ZC);
		nShiting += XM_4;

		xywhsf(pos->xmS4, nShiting, i * nLY + ny, XM_5, nLY, LS_9, FS, AM_ZC);
		nShiting += XM_5;

		xywhsf(pos->xmS5, nShiting, i * nLY + ny, XM_6, nLY, LS_9, FS, AM_ZC);
		nShiting += XM_6;

		xywhsf(pos->xmQt, nShiting, i * nLY + ny, XM_7, nLY, LS_9, FS, AM_ZC);
		nShiting += XM_7;
		i++;
	}
